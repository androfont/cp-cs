...
[AlgorithmInfo(
    ...
    timeComplexity: ComplexityValues.Cubic,
    memoryComplexity: ComplexityValues.Cuadratic,
    new[] {
        AlgorithmTags.Adhoc
    }
)]
class P06
{
    ...
    static bool[,] grid = new bool[1000, 1000];
    static int[,] gridBright = new int[1000, 1000];
    ...
    internal static void Run()
    {
        ...
        while (input != null)
        {
            lightsOn += UpdateGrid(input);
            UpdateBright(input);
            input = Console.ReadLine();
        }
        var bright = 0;
        foreach (var item in gridBright)
        {
            bright += item;
        }
        output.WriteLine($"Lights on: {lightsOn}");
        output.WriteLine($"Bright intensity: {bright}");
        output.Flush();
    }

    private static void UpdateBright(string input)
    {
        var command = ParseCommand(input);
        int[] coords1, coords2;
        ExtractCoords(input, out coords1, out coords2);
        switch (command)
        {
            case LightCommand.On:
                ExecuteTone(coords1, coords2, (r, c) =>
                {
                    gridBright[r, c]++;
                });
                break;
            case LightCommand.Off:
                ExecuteTone(coords1, coords2, (r, c) =>
                {
                    gridBright[r, c] = Math.Max(0, gridBright[r, c] - 1);
                });
                break;
    ...
    private static void ExecuteTone(int[] coords1, int[] coords2, Action<int, int> run)
    {
        int rowFrom = Math.Min(coords1[0], coords2[0]);
        int rowTo = Math.Max(coords1[0], coords2[0]);

        int columnFrom = Math.Min(coords1[1], coords2[1]);
        int columnTo = Math.Max(coords1[1], coords2[1]);
    ...
    static int UpdateGrid(string input)
    {
        var command = ParseCommand(input);
        int[] coords1, coords2;
        ExtractCoords(input, out coords1, out coords2);
        ...
        {
            case LightCommand.On:
                return Execute(coords1, coords2, (r, c) =>
                {
                    if (!grid[r, c])
                    {
                        grid[r, c] = true;
    ...
    private static void ExtractCoords(string input, out int[] coords1, out int[] coords2)
    {
        var commandArgs = input.Split(' ');
        coords1 = Array.ConvertAll(commandArgs[^1].Split(','), new Converter<string, int>(str => int.Parse(str)));
        coords2 = Array.ConvertAll(commandArgs[^3].Split(','), new Converter<string, int>(str => int.Parse(str)));
    }

    static int Execute(int[] vertex, int[] vertex1, Func<int, int, int> run)
    {
        int rowFrom = Math.Min(vertex[0], vertex1[0]);
        int rowTo = Math.Max(vertex[0], vertex1[0]);

        int columnFrom = Math.Min(vertex[1], vertex1[1]);
        int columnTo = Math.Max(vertex[1], vertex1[1]);
    ...
    static LightCommand ParseCommand(string command)
    {
    ...
    {
        Console.Write(@"toggle 461,550 through 564,900
